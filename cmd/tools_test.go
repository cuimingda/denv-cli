...
func TestNewInstallCmdWithServicePanicsWhenPlannerMissing(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatalf("expected NewInstallCmdWithService to panic when context is nil")
		}
	}()
	_ = NewInstallCmdWithService(nil)
}
...
func TestRootHasListCommand(t *testing.T) {
	cmd := NewRootCmdWithContext(NewCLIContextWithRuntime(testRuntime()))
	found := false
	installFound := false
	outdatedFound := false
	updateFound := false
...
func TestUpdateCommandOutputsDoingProgress(t *testing.T) {
	...
	cmd := NewUpdateCmdWithService(testCommandService())
	out := &bytes.Buffer{}
	errOut := &bytes.Buffer{}
	cmd.SetOut(out)
	cmd.SetErr(errOut)
